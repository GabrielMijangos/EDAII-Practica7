...
void dfs_traverse( Graph* g, Vertex* v , size_t* time)
{
   *time += 1;
...
void dfs( Graph* g, int start )
{
   ...
   for( int i = 0; i < Graph_GetLen( g ); ++i )
   {
      ...
      //tiempo de inicio y finalizacion en 0
      Vertex_SetDiscovery_time( v, time );
      Vertex_SetFinish_time( v, time );
   }
   ...
   dfs_traverse( g, Graph_GetVertexByKey( g, start ), &time );
}


int dfs_topol_traverse(Graph* g, Vertex* v, int* time, List* list)
{
    *time += 1;
    v->discovery_time = *time;
    v->color = GRAY;
    ...
    *time += 1;
    ...
    List_Push_back(list, Vertex_GetData(v), 0.0);
...
void dfs_topol(Graph* g, Vertex* start)
{
    ...
    //Primera fase: realizar DFS desde el vértice de inicio
    dfs_topol_traverse( g, start, &time_, list );

    //Segunda fase: procesar nodos no visitados
    for(  int i = 0; i < Graph_GetLen(g); ++i )
    {
        Vertex* v = Graph_GetVertexByIndex(g, i);

        if( Vertex_GetColor(v) == WHITE ) // Si el nodo no ha sido visitado
        {
            dfs_topol_traverse(g, v, &time_, list); // Hacer DFS sobre ese nodo
        }
    }
...
void Graph_Print_dfs(Graph* g, int depth)
{
   for (int i = 0; i < g->len; ++i)
   {
      ...
      printf("[%d] %d -- (Discovery Time: %d, Finish Time: %d) => ",
             i, vertex->data, vertex->discovery_time, vertex->finish_time);
      ...
      if (vertex->neighbors)
      {
         for (List_Cursor_front(vertex->neighbors); 
              ...
              List_Cursor_next(vertex->neighbors))
         {
            Data neighbor_data = List_Cursor_get(vertex->neighbors);
            int neighbor_idx = neighbor_data.index;
...
int main()
{
   ...
   int idx_vertex_start = 0;
   dfs_topol( grafo, Graph_GetVertexByIndex(grafo,idx_vertex_start) );
